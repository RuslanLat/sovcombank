...
class Authenticate:
    ...
    def _check_pw(self) -> bool:
        ...
        return self.password == self.credentials['usernames'][self.username]['password']
    ...
    def login(self, form_name: str, location: str='main') -> tuple:
        ...
        if not st.session_state['authentication_status']:
            ...
            if not st.session_state['authentication_status']:
                ...
                login_form.image(image, width=200)
                login_form.write("##")
                login_form.subheader(form_name)
                self.username = login_form.text_input('Имя пользователя', placeholder='Введите имя пользователя ...')
                st.session_state['username'] = self.username
                self.password = login_form.text_input('Пароль', placeholder='Введите пароль ...', type='password')

                if login_form.form_submit_button('Войти'):
